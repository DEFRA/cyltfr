...
CREATE TABLE u_ltfri.rofrs_england_bv_bng_valid TABLESPACE postgis_tables AS WITH
	make_valid(prob_4band,suitability,pub_date,risk_for_insurance_sop,shape_length,shape_area,wkb_geometry) AS
		(SELECT prob_4band,suitability,pub_date,'',shape_length,shape_area,
...
ALTER TABLE u_ltfri.rofrs_england_bv_bng_valid ALTER COLUMN risk_for_insurance_sop TYPE character varying;
ALTER TABLE u_ltfri.rofrs_england_bv_bng_valid ALTER COLUMN wkb_geometry TYPE geometry(Multipolygon,27700) USING ST_Force2D(ST_Multi(wkb_geometry));
