...
SELECT ogc_fid, reason(ST_IsValidDetail(wkb_geometry)) FROM u_ltfri.extra_info_bv_bng WHERE ST_IsValid(wkb_geometry) = false;

\echo 'Flood Alert Areas - IsValid'

\o /home/ubuntu/ltfri/spatial-data-loader/reports/flood_alert_area.txt

SELECT ogc_fid, reason(ST_IsValidDetail(wkb_geometry)) FROM u_ltfri.flood_alert_area_bv_bng WHERE ST_IsValid(wkb_geometry) = false;
...
SELECT ogc_fid, reason(ST_IsValidDetail(wkb_geometry)) FROM u_ltfri.rof_reservoir_extent_bv_bng WHERE ST_IsValid(wkb_geometry) = false;
...
SELECT ogc_fid, reason(ST_IsValidDetail(wkb_geometry)) FROM u_ltfri.sea_level_depth_gauges_bv_bng WHERE ST_IsValid(wkb_geometry) = false;
