...
module.exports = {
  ...
  assertOutcomeGW: function (riskDetailPage, data) {
    riskDetailPage.getInfo(function (info) {
      riskDetailPage.assert.equal(info.status, data.outcome)
      riskDetailPage.assert.equal(info.riverAndSeaRisk, data.riverAndSeaRisk)
      riskDetailPage.assert.equal(info.surfaceWaterRisk, data.surfaceWaterRisk)
      ...
      // Reservoir tests
      riskDetailPage.assert.elementPresent('@groundwaterPanel')
      riskDetailPage.assert.containsText('@groundwaterPanel', 'High groundwater levels may cause flooding')
      riskDetailPage.assert.containsText('@groundwaterText', 'Flooding and disruption is possible in the local area when groundwater levels are high.')
    })
  }
}
