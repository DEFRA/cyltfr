...
begin
  ...
  begin
    select (select array_agg(faa.fwis_code) as fwis_code from u_ltfri.flood_alert_area_bv_bng faa where st_intersects(st_setsrid(st_makepoint(_x, _y), 27700), faa.wkb_geometry)) as flood_alert_area into flood_alert_area_result;
  exception when others then
  ...
  begin
    ...
    select (select array_agg(fwa.fwis_code) as fwis_code from u_ltfri.flood_warning_area_bv_bng fwa where st_intersects(st_setsrid(st_makepoint(_x, _y), 27700), fwa.wkb_geometry)) as flood_warning_area into flood_warning_area_result;
  exception when others then
