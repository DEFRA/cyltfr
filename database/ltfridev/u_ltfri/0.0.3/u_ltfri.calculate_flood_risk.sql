...
begin
  ...
  exception when others then
    select 'Error' as in_england into within_england_result;
  end;
  ...
  exception when others then
    select 'Error' as in_flood_alert_area into flood_alert_area_result;
  end;
  begin
  ...
  exception when others then
    select 'Error' as in_flood_warning_area into flood_warning_area_result;
  end;
  ...
  exception when others then
    select 'Error' as surface_water_suitability into high_surface_water_result;
  end;
  begin
  ...
  exception when others then
    select 'Error' as lead_local_flood_authority into lead_local_flood_authority_result;
  end;
  begin
  ...
  exception when others then
    select 'Error' as info into extra_info_result;
  end;
  ...
  exception when others then
    rofrs_error := true;
    select 'Error' as prob4_band into rofrs_result;
  end;
  ...
  exception when others then
    ...
    select 'Error' as risk_desig into reservoir_result;		
  end;
