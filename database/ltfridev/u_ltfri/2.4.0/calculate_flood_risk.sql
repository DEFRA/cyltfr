...
-- 2.4.0    LTFRI-479 mcollins  Add risk override to local information (extra_info)
...
begin
  ...
  begin
    ...
    select (select array_agg(fwa.fws_tacode) as fwis_code from u_ltfri.flood_warning_area_bv_bng fwa where st_intersects(st_setsrid(st_makepoint(_x, _y), 27700), fwa.wkb_geometry)) as flood_warning_area into flood_warning_area_result;
  exception when others then
  ...
  begin
    ...
    select info, apply, riskoverride from u_ltfri.extra_info_bv_bng l 
  ...
  begin
     ...
     select array_to_json(array_agg(row_to_json(t))) as dry_reservoir
        from (
	   select r.reservoir, r.ngr, r.llfa_name, r.undertaker, r.risk_designation, r.comments from u_ltfri.rof_reservoir_extent_dry_bv_bng r where st_intersects(st_setsrid(st_makepoint(_x, _y), 27700), r.wkb_geometry)
	) t into dry_reservoir_result;
     in_dry_reservoir_risk_area := found;
  exception when others then
    dry_reservoir_error := true;
  end;

  begin
     -- Wet reservoirs risk point in polygon query.
     select array_to_json(array_agg(row_to_json(t))) as wet_reservoir
        from (
	   select r.reservoir, r.ngr, r.llfa_name, r.undertaker, r.risk_designation, r.comments from u_ltfri.rof_reservoir_extent_wet_bv_bng r where st_intersects(st_setsrid(st_makepoint(_x, _y), 27700), r.wkb_geometry)
	) t into wet_reservoir_result;
     in_wet_reservoir_risk_area := found;
  exception when others then
    wet_reservoir_error := true;
  end;
  ...
  with data as (
    select
      ...
      flood_alert_area_result.flood_alert_area,
      flood_warning_area_result.flood_warning_area,
      ...
      end as dry_reservoir_risk,

      case in_wet_reservoir_risk_area
        when true then
